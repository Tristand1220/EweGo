...
def load_audio_timestamps_csv(audio_path: Path,
    ...
    candidates = [
        ...
        audio_path.parent / (audio_path.stem + ".timestamps.csv"),
    ]
    for path in candidates:
        if not path.exists():
            continue
...
# ---------------------------------------------------------------------------
# Interactive Plotly — combined AV figure (brightness + audio amplitude)
# ---------------------------------------------------------------------------
...
def make_av_html(
    ...
    utc2: np.ndarray, bright2: np.ndarray,
    # Audio
    audio_samples: np.ndarray, audio_sr: int,
    audio_block_utc: "np.ndarray | None",   # per-block UTC from CSV  (preferred)
    audio_block_samples: "np.ndarray | None",  # per-block sample indices from CSV
    audio_start_utc: float,                 # fallback scalar t0 if no CSV
    # Metadata
    params: dict | None, session_label: str,
):
    """
    Single Plotly HTML file with two stacked, x-axis-linked subplots:

      Top    — ROI brightness (0-255) for Camera 1 and Camera 2
               Each dot = one captured frame.
               Hover shows: camera, elapsed time (5 dp), UTC, frame number,
                            brightness value.

      Bottom — Audio peak amplitude envelope sampled at each audio block.
               Each dot = one audio callback block (~21 ms at 48 kHz / 1024).
               Hover shows: elapsed time (5 dp), UTC, sample index, peak amplitude.

    Both subplots share the same elapsed-time x-axis so zooming/panning
    one panel moves the other in lockstep.

    Audio amplitude is computed as peak(|samples|) within each callback block,
    which directly reflects the sawtooth volume ramp — identical philosophy to
    Plot 5 in the matplotlib audio figure but with per-block resolution
    (1024 samples / 48000 Hz ≈ 21.3 ms per point) instead of a fixed window.
    If the per-block timestamp CSV is available, each point is placed at its
    true monotonic-clock-derived UTC position rather than estimated from sample
    count alone.
    """
    if not PLOTLY_OK:
        print("[analyze_sync] plotly not installed — skipping interactive "
              "AV HTML output.  pip install plotly")
        return None

    try:
        from plotly.subplots import make_subplots
    except ImportError:
        print("[analyze_sync] plotly.subplots unavailable — skipping AV HTML.")
        return None

    # ---- Shared t0: earliest event across all sensors ----------------------
    t0 = min(utc1[0], utc2[0])
    if audio_block_utc is not None:
        t0 = min(t0, audio_block_utc[0])
    else:
        t0 = min(t0, audio_start_utc)

    rel1 = utc1 - t0
    rel2 = utc2 - t0

    # ---- Audio amplitude per block -----------------------------------------
    blocksize  = 1024   # matches recorder BLOCKSIZE
    n_ch       = audio_samples.shape[1] if audio_samples.ndim > 1 else 1
    mono       = audio_samples[:, 0] if n_ch > 1 else audio_samples
    # Normalise int32 → float if needed
    if mono.dtype == np.int32:
        mono = mono.astype(np.float64) / 2_147_483_648.0
    elif mono.dtype == np.int16:
        mono = mono.astype(np.float64) / 32768.0
    else:
        mono = mono.astype(np.float64)

    n_blocks    = len(mono) // blocksize
    blocks      = np.abs(mono[:n_blocks * blocksize].reshape(n_blocks, blocksize))
    peak_amp    = blocks.max(axis=1)   # shape (n_blocks,)

    # X positions for audio points
    if audio_block_utc is not None and len(audio_block_utc) >= n_blocks:
        # Use precise per-block monotonic timestamps from the CSV
        audio_utc_pts = audio_block_utc[:n_blocks]
        rel_audio     = audio_utc_pts - t0
        sample_idx    = audio_block_samples[:n_blocks]
        audio_source  = "per-block monotonic timestamps (CSV)"
    else:
        # Fall back to sample-count-based positions anchored to audio_start_utc
        block_centres_s = (np.arange(n_blocks) + 0.5) * blocksize / audio_sr
        audio_utc_pts   = audio_start_utc + block_centres_s
        rel_audio       = audio_utc_pts - t0
        sample_idx      = np.arange(n_blocks) * blocksize
        audio_source    = "estimated from sample count (no CSV found)"

    print(f"[analyze_sync] Audio amplitude: {n_blocks} blocks  source={audio_source}")

    # ---- Build hover text helpers ------------------------------------------
    def _video_hover(rel_s, utc_s, bright, cam_label):
        texts = []
        for i, (r, u, b) in enumerate(zip(rel_s, utc_s, bright)):
            utc_dt = datetime.fromtimestamp(u, tz=timezone.utc)
            texts.append(
                f"<b>{cam_label}</b><br>"
                f"Elapsed : {r:.5f} s<br>"
                f"UTC     : {utc_dt.strftime('%H:%M:%S.%f')[:-1]}<br>"
                f"Frame # : {i}<br>"
                f"Brightness: {b:.2f} / 255"
            )
        return texts

    def _audio_hover(rel_s, utc_s, samp_idx, amp):
        texts = []
        for r, u, si, a in zip(rel_s, utc_s, samp_idx, amp):
            utc_dt = datetime.fromtimestamp(u, tz=timezone.utc)
            texts.append(
                f"<b>Microphone</b><br>"
                f"Elapsed : {r:.5f} s<br>"
    ...
    # ---- Ground-truth envelope for audio panel -----------------------------
    gt_audio = None
    if params is not None and params.get("audio_envelope_freq_hz") is not None:
        gt_audio_raw = ground_truth_audio_envelope(audio_utc_pts, params)
        if gt_audio_raw is not None:
            raw    = params.get("_raw", {})
            volume = raw.get("audio", {}).get("volume", 0.8)
            gt_audio = gt_audio_raw * volume

    # ---- Ground truth for video panel --------------------------------------
    span_s = max(rel1[-1], rel2[-1])
    gt_video_x = gt_video_y = None
    if params is not None:
        gt_video_x = np.linspace(0, span_s, int(span_s * 200))
        gt_video_y = ground_truth_visual(gt_video_x + t0, params)
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "ROI Brightness — Camera 1 & Camera 2  (each dot = one frame)",
            "Audio Amplitude Envelope  (each dot = one callback block "
            f"≈ {blocksize/audio_sr*1000:.1f} ms)",
        ),
    )

    # ---- Top panel: video brightness ----------------------------------------
    if gt_video_x is not None:
        fig.add_trace(go.Scatter(
            x=gt_video_x, y=gt_video_y,
            mode="lines",
            name=f"Ground-truth ({params['visual_waveform']})",
            line=dict(color=TRUTH_COLOR, width=1.2, dash="dash"),
            opacity=0.5,
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines+markers",
        name="Camera 1",
        line=dict(color=CAM1_COLOR, width=1.0),
        marker=dict(color=CAM1_COLOR, size=5, line=dict(width=0)),
        opacity=0.9,
        text=_video_hover(rel1, utc1, bright1, "Camera 1"),
        hoverinfo="text",
    ), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="lines+markers",
        name="Camera 2",
        line=dict(color=CAM2_COLOR, width=1.0),
        marker=dict(color=CAM2_COLOR, size=5, line=dict(width=0)),
        opacity=0.9,
        text=_video_hover(rel2, utc2, bright2, "Camera 2"),
        hoverinfo="text",
    ), row=1, col=1)

    # ---- Bottom panel: audio amplitude --------------------------------------
    if gt_audio is not None:
        fig.add_trace(go.Scatter(
            x=rel_audio, y=gt_audio,
            mode="lines",
            name="Ground-truth envelope",
            line=dict(color=TRUTH_COLOR, width=1.2, dash="dash"),
            opacity=0.5,
            hoverinfo="skip",
        ), row=2, col=1)
    ...
    fig.add_trace(go.Scatter(
        x=rel_audio, y=peak_amp,
        mode="lines+markers",
        name="Microphone",
        line=dict(color=AUDIO_COLOR, width=1.0),
        marker=dict(color=AUDIO_COLOR, size=4, line=dict(width=0)),
        opacity=0.85,
        text=_audio_hover(rel_audio, audio_utc_pts, sample_idx, peak_amp),
        hoverinfo="text",
    ), row=2, col=1)
    ...
    if params is not None:
        freq   = params["visual_freq_hz"]
        title_extra = f"  ·  f = {freq:.4f} Hz  T = {1/freq:.2f} s"

    _dark_axis = dict(gridcolor="#2a2a2a", zerolinecolor="#333333",
                      linecolor="#333333")

    fig.update_layout(
        title=dict(
            text=(f"Audio/Visual Sync  ·  Session: {session_label}{title_extra}<br>"
                  f"<sup>Hover any point for exact elapsed time, UTC, "
                  f"frame/sample number</sup>"),
    ...
    fig.update_xaxes(
        title_text="Elapsed time (s)  —  drag to zoom, double-click to reset",
        row=2, col=1,
        **_dark_axis,
    )
    fig.update_xaxes(showticklabels=False, row=1, col=1, **_dark_axis)

    fig.update_yaxes(title_text="Brightness (0–255)", range=[-5, 265],
                     row=1, col=1, **_dark_axis)
    fig.update_yaxes(title_text="Peak amplitude",
                     row=2, col=1, **_dark_axis)
...
def main():
    ...
    if audio_path.exists():
        ...
        try:
            ...
            if audio_block_result is not None:
                ...
                audio_block_utc = audio_block_samples = None
                print("  [INFO] No per-block timestamp CSV found — audio "
                      "points will be placed by sample count from t0.")
            ...
            # --- Interactive AV Plotly figure --------------------------------
            if not args.no_html:
                print("[analyze_sync] Rendering interactive AV plot...")
                fig_av_html = make_av_html(
                    utc1, bright1, utc2, bright2,
                    audio_samples, audio_sr,
                    audio_block_utc, audio_block_samples,
                    audio_start,
                    params, session_label,
                )
            else:
    ...
    if args.output_prefix:
        ...
        av_html_out = Path(str(args.output_prefix) + "_av.html")
    else:
        ...
        av_html_out = session_dir / f"{session_label}_av.html"
    ...
    if fig_av_html is not None:
        fig_av_html.write_html(str(av_html_out), include_plotlyjs="cdn")
        print(f"[analyze_sync] Interactive AV plot saved → {av_html_out}")
        print("  Open in a browser — zoom either panel and both track "
              "together. Hover for elapsed time, UTC, frame/sample number.")
